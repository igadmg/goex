...
type Decoder struct {
	...
	ignoreDepth            int
	TypeTransformFunctions map[int]func(any) any
}
...
func (dec *Decoder) Make() {
	...
	dec.TypeTransformFunctions = map[int]func(any) any{}
}
	...
	if id.isBuiltin() {
		...
		case reflect.Struct:
			...
			for _, f := range w.StructT.Field {
				size += dec.typeSize(f.Id)
			}
			return size
		case reflect.Map:
			return uint32(unsafeex.Sizeof[map[any]any]())
		}
	}
